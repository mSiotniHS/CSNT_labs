...
try
{
	...
	while (true)
	{
		...
		var thread = new Thread(() => ServeClient(handler, clientCounter++));
		thread.Start();
	}
}
catch (Exception e)
...
void ServeClient(Socket handler, int clientId)
{
	while (true)
	{
		...
		Console.WriteLine($"[{DateTime.Now.ToShortTimeString()}|от №{clientId}] {builder} ({totalBytes} bytes)");
